...
const Blockchain = {
    ...
    performTransaction: async (total) => {

        const sellerAddress = '0x2516F83D12E50A1980caFd962AB73226319D5AF7'; // Replace 'SELLER_ADDRESS' with the actual address of the seller
        ...
        try {
            const block =false;
            const etherToDollarRate = 200;
            const totalPrice = total / etherToDollarRate;
            const userAddress=web3.eth.getAccounts;
            userAddress = userAddress[1];

contractInstance.methods.buy(userAddress, totalPrice).send({
    from: account.address,
    value: totalPrice,
